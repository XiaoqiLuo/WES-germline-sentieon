...
task SentieonFastqToVcf {
    ...
    # String out_vcf = sample_name + ".gvcf.gz"
    # String out_vcf_idx = sample_name + ".gvcf.gz.tbi"
    String dollar = "$"
    ...
    command <<<
        ...
        # ******************************************
        ## Step1 Mapping reads with BWA-MEM, sorting
        # ******************************************
        #The results of this call are dependent on the number of threads used. To have number of threads independent results, add chunk size option -K 10000000 

        ( sentieon bwa mem -R ~{read_group_info} -K 100000000 -Y -t $(nproc) ~{ref_fasta} ~{fastq1} ~{fastq2} || echo -n 'error' ) \
        ...
        sentieon plot GCBias -o ~{sample_name}.gc-report.pdf ~{sample_name}.gc_metrics.txt
        sentieon plot QualDistribution -o ~{sample_name}.qd-report.pdf ~{sample_name}.qd_metrics.txt
        sentieon plot MeanQualityByCycle -o ~{sample_name}.mq-report.pdf ~{sample_name}.mq_metrics.txt
        sentieon plot InsertSizeMetricAlgo -o ~{sample_name}.is-report.pdf ~{sample_name}.is_metrics.txt
        ...
        # ******************************************
        # Step3 Remove Duplicate Reads. It is possible
        # to remove instead of mark duplicates
        # by adding the --rmdup option in Dedup
        # ******************************************
        sentieon driver -t $(nproc) -i $next_bam --algo LocusCollector \
        ...
        # ******************************************
        #Step5 Variant caller
        # ******************************************
        sentieon driver -r ~{ref_fasta} -t $(nproc) \
            --interval_padding 100 -i $next_bam -q ~{bqsr_table} \
            --algo Haplotyper --emit_mode gvcf -d ~{dbsnp_vcf} ~{out_gvcf}
        sentieon driver -r ~{ref_fasta} -t $(nproc) \
            -i $next_bam -q ~{bqsr_table} \
            --algo ReadWriter ~{sample_name}.recaled.bam
